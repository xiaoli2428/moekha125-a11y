...
# prepare_and_open_pr.sh
# Creates a branch, adds MultiTimezoneClock React + Vue components and shared timezone list + CSS,
# commits files (one commit per file), pushes branch and opens a PR using gh.
#
# Usage:
#   chmod +x prepare_and_open_pr.sh
#   ./prepare_and_open_pr.sh --yes
#
# Requirements:
# - Run from repository root (where .git exists)
# - git and gh installed and authenticated (gh auth login)
# - You must have push rights to the repo
#
# Defaults (change variables below if you want different names)
OWNER="xiaoli2428"
REPO="moekha125-a11y"
...
echo "Planned branch base: ${BRANCH_BASE}"
echo

if [[ "${AUTO_YES}" != "true" ]]; then
  read -r -p "Proceed to create branch, add files, push and open PR? (y/N) " CONFIRM
  if [[ "${CONFIRM,,}" != "y" ]]; then
    echo "Aborted."
    exit 0
  fi
...
# Ensure base branch is present and up-to-date
...
# Determine branch name; if remote branch exists, append timestamp
if git ls-remote --exit-code --heads origin "${BRANCH}" >/dev/null 2>&1; then
  TS=$(date +%Y%m%d%H%M%S)
  BRANCH="${BRANCH_BASE}-${TS}"
  echo "Remote branch ${BRANCH_BASE} exists; using ${BRANCH} instead."
fi
 ...
 *
 * Props (added/important):
 *  - syncStrategy: "replace" | "merge" | "lww" | "per-item" (default: "per-item")
    ...
    function onStorage(e) {
      ...
      if (props.syncStrategy === "replace") {
        ...
        for (const [tz, rIt] of remote.items) {
          const lIt = merged.get(tz);
          if (!lIt) merged.set(tz, { tz: rIt.tz, deleted: !!rIt.deleted, ts: rIt.ts });
          else if ((rIt.ts || 0) > (lIt.ts || 0)) merged.set(tz, { tz: rIt.tz, deleted: !!rIt.deleted, ts: rIt.ts });
        }
...
# Create PR using gh
echo "Creating PR..."
PR_URL=$(gh pr create --base "${BASE_BRANCH}" --head "${BRANCH}" --title "${PR_TITLE}" --body "${PR_BODY}" --json url --jq .url 2>/dev/null || true)

if [[ -n "${PR_URL}" ]]; then
  echo "PR created: ${PR_URL}"
else
  echo "Non-interactive PR creation failed or opened browser. Trying interactive (may open browser)..."
  gh pr create --base "${BASE_BRANCH}" --head "${BRANCH}" --title "${PR_TITLE}" --body "${PR_BODY}" --web || true
  echo "If a browser opened, complete the PR there. Otherwise run the gh pr create command shown in the script output above."
fi

echo
echo "Done. Branch: ${BRANCH}"
echo "Review the PR and adjust as needed."