...
-- Insert 5 default levels if table is empty
INSERT INTO trading_levels (name, min_amount, max_amount, payout_percentage, is_active)
SELECT name, min_amount, max_amount, payout_percentage, is_active
FROM (VALUES
...
) AS v(name, min_amount, max_amount, payout_percentage, is_active)
WHERE NOT EXISTS (SELECT 1 FROM trading_levels);
...
-- Insert 5 default AI arbitrage levels if table is empty
INSERT INTO ai_arbitrage_levels (name, min_amount, max_amount, duration_seconds, profit_percentage, loss_percentage, is_active)
SELECT name, min_amount, max_amount, duration_seconds, profit_percentage, loss_percentage, is_active
FROM (VALUES
...
) AS v(name, min_amount, max_amount, duration_seconds, profit_percentage, loss_percentage, is_active)
WHERE NOT EXISTS (SELECT 1 FROM ai_arbitrage_levels);
...
-- Optional: Update a test user if needed (commented out to avoid errors if user doesn't exist)
-- UPDATE users 
